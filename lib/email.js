...
function getFromEmail() {
  return process.env.FROM_EMAIL || "Dream AI Sales Engine <no-reply@example.com>";
}
...
Your Dream AI Sales Pipeline Diagnostic Report is attached.

It shows where your business may be losing leads, bookings, and revenue - and which AI agents to deploy first.

After reviewing it, you can book your free Dream AI Sales Audit here:
${bookingLink}

Regards,
Dream AI Sales Engine`;

  return sendEmail({
    to: prospect.email,
    subject: "Your Dream AI Sales Pipeline Diagnostic Report",
    text,
    html: lineBreaksToHtml(text),
    attachments: [
      {
        filename: "ai-sales-pipeline-diagnostic-report.pdf",
        content: pdfBuffer
      }
    ]
  });
...
async function sendAdminNotification({ prospect, score, answers, bookingLink }) {
  ...
  const text = `New Dream AI Sales Pipeline Diagnostic Submission
...
${JSON.stringify(answers, null, 2)}`;
  ...
  return sendEmail({
    ...
    subject: "New Dream AI Sales Pipeline Diagnostic Submission",
    text,
